...
<html>
    ...
    <body>​
        ...
        <%
            ...
            ArrayList<CheckInOutBean> dueTimeList = (ArrayList<CheckInOutBean>) request.getAttribute("dueTimeList");String limit = (String) request.getAttribute("limit");
            ...
            <div id="contentwrapper">​
                <div id="contentcolumn">​
                    <div class="innertube">
                        ...
                        <br>
                        <div class="btn_pos">
                            <a class="<%=isCheckInActive%>" 
                               href="CheckInOutController?action=viewCheckIn&limit=10">Check-In</a>
                            <a class="<%=isCheckOutActive%>" 
                               href="CheckInOutController?action=viewCheckOut&limit=10">Check-Out</a>
                        </div>
                        <div>
                            <ipage:page fullsize="<%=records.size()%>" limit="<%=procLimit%>" page="<%=type%>"  />  
                        </div>
                        <form action="CheckInOutController" class='s_bar_con'>
                            <input type="hidden" name="action" value="search">
                            <input type="hidden" name="limit" value="10">
                            <input type="hidden" name="inOrOut" value="<%=type%>">
                        ...
                        <div id="usertable-container">
                            <proc:procBrw records="<%=records%>" equName="<%=equName%>" limit="<%=procLimit%>" page="<%=type%>" dueTimeList="<%=dueTimeList%>" />  
                        </div>
                    </div>
                </div>​
            </div>​
    ...
    <script>
        function isSubmit(e) {
            ...
            var r = confirm(txt + " " + text + " ?");
            console.log(txt + " : " + id);
            if (r == true) {
                window.location.href = "CheckInOutController?action=" + txt + "&eid=" + eid + "&stock=" + stock + "&request=" + txt + "&id=" + id;
            } else {
                return;
            }
        }
